...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "style-analysis") {
      ...
      try {
        const { res: backendRes, json: responseData } = await fetchJsonWithTimeout(
          `${BACKEND_BASE_URL}/api/style/analyze`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!backendRes.ok) {
          console.error("[tryon-proxy] Style analysis backend error:", responseData);
          return new Response(
            JSON.stringify({ 
              success: false, 
              error: responseData?.error || "스타일 분석 중 오류가 발생했습니다" 
            }),
        ...
        if (!responseData?.analysis && !responseData?.celebrityMatch) {
          console.error("[tryon-proxy] Style analysis invalid response structure:", responseData);
          return new Response(
            JSON.stringify({ 
              success: false, 
              error: "AI 응답 형식이 올바르지 않습니다. 잠시 후 다시 시도해주세요." 
            }),
            { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } },
          );
        }
        ...
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: isTimeout ? "분석 시간이 초과되었습니다. 다시 시도해주세요." : "스타일 분석 서버 연결에 실패했습니다" 
          }),
          { status: 504, headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
